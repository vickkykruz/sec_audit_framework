...
def generate_pdf(scan_result: ScanResult, output_path: str, profile: str = "generic", drift_report=None, simulation_result=None, patch_results=None, fix_results=None, dry_run: bool = False) -> None:
    """
    ...
    if fix_results:
        story.append(Spacer(1, 14))
        section_title = "Auto-Fix Dry-Run Plan" if dry_run else "Auto-Fix Results"
        story.extend(_section(section_title, styles))
        ...
        if fixed_n > 0:
            ...
            if dry_run:
                note = (f"<b>{fixed_n}</b> check(s) would be fixed. "
                        "Review above, then run without <b>--dry-run</b> to apply.")
                note_color = ACCENT
            else:
                note = (f"<b>{fixed_n}</b> fix(es) applied to the server. "
                        "Run <b>--compare-last</b> to verify improvements.")
