...
# Cache so ipinfo is only called once per session
...
def prompt_first_run() -> None:
    ...
    # when --history or --telemetry flags are used without a scan
    if opted_in:
        # Get location synchronously here — program exits after this
        loc = _get_location()
        _post(ACTIVITY_URL, {
            "type":         "tool_installed",
            "page":         "/cli/install",
        ...
        # Also fire track for the install event
        _post(TRACK_URL, {
            "page":         "/cli/install",
            "country":      loc["country"],
            "country_code": loc["country_code"],
...
def _get_location() -> dict:
    ...
    try:
        ...
        data = json.loads(raw)
        _location_cache = {
            "country":      data.get("country", "XX"),   # ISO2 code e.g. "GB"
...
def _fire_track(page: str) -> None:
    """
    POST to /api/track with real location data from ipinfo.io.
    Runs inside a background thread so it never blocks the scan.
    Location is cached after the first call.
    """
    def _do_track():
        loc = _get_location()
        _post(TRACK_URL, {
